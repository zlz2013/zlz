...
class CoffeeDao:
    ...
    def get_last_eqinfo(self):
        sql='''
        select * from manage_eqstate where id in(select max(id) from manage_eqstate group by machine_id)
        '''
        result=self.db_helper.do_query(sql)
        return result

    def get_categray_info_by_machine_id(self,m_id):
        sql='''
        select a.* from manage_eqtype a,manage_eqinfo b where b.machine_id="%s" and a.category_id=b.category_id
        '''%m_id
        result = self.db_helper.do_query(sql)
        return result
    ...
    def add_machine_warnings(self, id, msg):  #插入对象
        ...
        if not result:
            sql = '''
            insert into manage_eqwarning(id, MACHINE_ID, alter_msg, check_time) values(NULL, %d, '%s', now())
            ''' % (id, msg)
        else:
